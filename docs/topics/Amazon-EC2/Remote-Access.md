...
This one is simple, assuming that you have the identity file saved locally. Simply run the command `#!sh {% raw %}ssh {{ username }}@{{ ec2host }} -i {{ path_to_identity_file }}{% endraw %}`
...
<details>
<summary>*Why use this script?*</summary>
...
2. Download the script: `#!sh wget https://raw.githubusercontent.com/eliminmax/mini-utils/main/ec2-ssh-config.sh`

3. Read over the script, and ensure that you can trust it.

4. Mark it as executable with `#!sh chmod u+x ec2-ssh-config.ssh`

5. Run the script with `#!sh ./ec2-ssh-config.ssh`, and enter the path to the identity file.
